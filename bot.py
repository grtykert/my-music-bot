...
def send_welcome(message):
    bot.reply_to(message, "👋 Привет! Напиши название трека, и я найду его на YouTube 🎵")
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for i, track in enumerate(tracks):
                ...
                markup.add(InlineKeyboardButton(text=f"🎵 {title}", callback_data=f"yt_{i}"))
...
def callback_send_track(call):
    ...
    # Отправляем сообщение со ссылкой — Telegram сам красиво развернет её в плеер
